...
A Tenant Application represents an isolated machine identity (client) under your organization. Once set up and assigned a subscription plan, its credentials are used to obtain short-lived secure access tokens to communicate with the high-performance TrueParser Parsing API.
...
An application must have an active subscription plan assigned to process files. Based on the selected plan, any access tokens issued to your application carry corresponding **plan entitlements** (e.g., parsing format support, document limits, and retention policy).
...
By default, files parsed by TrueParser are held in secure, platform-managed storage. However, if your enterprise requires direct data custody and compliance, you can bring your own S3-compatible storage bucket.
