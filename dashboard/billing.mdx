...
<Note>
  No plan assigned: the app can still issue tokens, but no `entitlements`
  claim is added. Retired plan assigned: token issuance is blocked.
</Note>
...
Only one app per tenant can use a free plan.
...
- assign a usable plan if you need the `entitlements` claim
- keep only one app per tenant on a free plan
