...
| :--- | :--- | :--- |
| **`Pending`** | The invitation is created and the email is queued. | The token is active. The invitation expires automatically after **24 hours**. |
...
| **`Expired`** | The 24-hour validation window passed before acceptance. | Sign-in is blocked. Admins can click **Resend** to renew the token. |
...
<Warning>
  Outstanding invitations can be cancelled at any time by an administrator. Once cancelled, any attempt to use the invitation token will be rejected by the backend with a `404 Not Found` exception.
</Warning>
