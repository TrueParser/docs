...
To resolve this, TrueParser requires an explicit `csvRoute` at ingestion time:
- `Spatial`: Routes to the GIS family.
- `Tabular`: Routes to the Office family.

This ensures that your spatial CSV is processed with the full GIS engine (geometry, WKT, BBOX) while your financial reports are handled by the tabular engine.
